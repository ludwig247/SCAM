...
SCAM::FunctionFactory::FunctionFactory(const std::map<int, CFGNode *> &controlFlowMap, Function  * function) {
    ...
    for (auto &&section : valuePropagation.getNewSectionPathMap()) {
        for (auto &&path : section.second) {
            returnValueConditionList.push_back((std::make_pair((Return*)path->getStmtList().back(),path->getConditionList())));
            //returnValueConditionMap.insert(std::make_pair((Return*)path->getStmtList().back(),path->getConditionList()));
        }
    }
