...
void SCAM::ModelFactory::addBehavior(SCAM::Module *module, clang::CXXRecordDecl *decl) {
    ...
    } else throw std::runtime_error("-E- More sections declared as used. Please either adjust section enum or behavior describtion");
...
void SCAM::ModelFactory::addVariables(SCAM::Module *module, clang::CXXRecordDecl *decl) {
    ...
    //Add datatypes to the module
    //Datatypes
    std::map<std::string, clang::QualType> typemap;
    for(auto var:  findVariables.getVariableMap()){
        typemap.insert(std::make_pair(var.first,var.second->getType()));
    }
    this->addEnumType(typemap);
    this->addCompoundDataType(module, findVariables.getVariableMap());

    //Initial Values
    FindInitalValues findInitalValues(decl, findVariables.getVariableMap());

    //Add members to module
    for (auto variable: findVariables.getVariableTypeMap()) {
        auto dataTypeMap = DataTypes::getDataTypeMap();
        //Check DataType validity
        if (dataTypeMap.find(variable.second) == dataTypeMap.end()) {
            throw std::runtime_error(variable.second + ": unknown type for variable" + variable.first);
        }
        DataType *type = DataTypes::getDataTypeMap().at(variable.second);
        ...
        if (type->isCompoundType()) {
            ...
            const auto &intitalValMap = findInitalValues.getVariableInitialMap();
...
//! Adds a new DataType to the model if it does not exist
void SCAM::ModelFactory::addCompoundDataType(SCAM::Module *module, std::map<std::string, clang::FieldDecl *> memberMap) {
    for (auto var: memberMap) {
        //Is not a built-in type?
        if (var.second->getType()->isStructureType()) {
            //Check for valid compound datatype
            FindNewDatatype newDatatype(var.second->getType()->getAsCXXRecordDecl());
            if (newDatatype.isValidDatatype()) {

                auto dataTypeMap = DataTypes::getDataTypeMap();

                std::string typeName = newDatatype.getTypeName();
                //Does the Datatype already exist?
                if (dataTypeMap.find(typeName) == dataTypeMap.end()) {
                    //Check for valid type
                    //Create new Datatype
                    DataType *dataType = new DataType(newDatatype.getTypeName());
                    for (auto member: newDatatype.getSubVarMap()) {
                        //Name
                        std::string name = member.first;
                        //Type
                        std::string typeString = member.second;
                        //Only int and bool are supported for compound datatypes as members
                        if (!DataTypes::getDataType(typeString)->isBuiltInType() && !DataTypes::getDataType(typeString)->isEnumType()) {
                            throw std::runtime_error(typeString + " is not supported for compound Datatypes");
                        }
                        //Add new member to Module
                        dataType->addSubVar(name, DataTypes::getDataType(typeString));
                    }
                    DataTypes::addDataType(dataType);
                }
            }
        }
    }
...
void SCAM::ModelFactory::addEnumType(const std::map<std::string, QualType> &typMap) {
    for (auto type: typMap) {
        if (type.second->isEnumeralType()) {
            const clang::EnumType *enumType = type.second->getAs<clang::EnumType>();
            //Is dataType already in Map?
            if (!DataTypes::isDataType(enumType->getDecl()->getName())) {
                ...
                if (enumType->getDecl()->getName().str() != "Sections") {
                    ...
                    SCAM::DataType *newType = new DataType(enumType->getDecl()->getName());
                    //Add each enum_value
...
void SCAM::ModelFactory::addFunctions(SCAM::Module *module, CXXRecordDecl *pDecl) {
    return; //Functions are disabled for this version.
    FindFunctions findFunction(pDecl);
    //Add enum type
    std::map<std::string, QualType> typeMap;
    for(auto func: findFunction.getFunctionMap()){
        typeMap.insert(std::make_pair(func.first,func.second->getResultType()));
    }
    addEnumType(typeMap);
