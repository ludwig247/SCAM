...
SCAM::Variable::Variable(std::string name, SCAM::DataType *type,const SCAM::ConstValue *initialValue):
        dataType(type),
        mIsSubVar(false),
        parent(nullptr),
    ...
    if (initialValue->getDataType() != type) throw std::runtime_error("Variable Constructor, initial_value type does not match the varialbe type");
...
const SCAM::ConstValue *SCAM::Variable::getInitialValue() const {
    if(this->getSubVarList().size() != 0) throw std::runtime_error(this->getName() + ": Compound doesn't have a initial value");
    return initialValue;
}
...
std::string SCAM::Variable::getFullName() {
    if(this->isSubVar()){
