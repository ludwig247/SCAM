...
namespace SCAM{
    ...
    public:
        ...
        void setSections(DataType *sectionType); //default initialSection, first in list
        bool isSectionValue(const std::string n) const;
    ...
    private:
        ...
        std::map<int, State *> stateMapOptimized; //! Stores a map of the abstract states and the connecting operations
        std::map<Operation*, Path * > operationPathMap;



        void setSectionVariable(Variable *sectionVariable);
        void setNextSectionVariable(Variable *nextsectionVariable);
    };
}
