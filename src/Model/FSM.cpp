...
void SCAM::FSM::setSectionVariable(SCAM::Variable *sectionVariable) {

    FSM::sectionVariable = sectionVariable;
}

void SCAM::FSM::setNextSectionVariable(SCAM::Variable *nextsectionVariable) {
    FSM::nextSectionVariable = nextsectionVariable;
}
...
bool SCAM::FSM::isSectionValue(const std::string n) const {
    for (auto &&sectionNamePair : sectionVariable->getDataType()->getEnumValueMap()) {
        if (sectionNamePair.first == n)
            return true;
    }
    return false;
}
...
void SCAM::FSM::setSections(DataType *sectionType) {
    if(!sectionType->isEnumType()) throw std::runtime_error("Section Type is wrong: not an enumType");
    if(sectionType->getName() != this->module->getName()+"_SECTIONS") throw std::runtime_error(this->module->getName() +": Wrong section type " + sectionType->getName());

    this->setSectionVariable(new Variable("section",sectionType)); //default init);
    this->setNextSectionVariable(new Variable("section",sectionType)); //default init);
}
