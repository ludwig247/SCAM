            ...
            for (auto path2 : section.second) {
                ...
                for (int k = 0; k < path1->getStmtList().size() && isEqual; k++) {
                    ...
                    if (path1->getStmtList().at(k) != path2->getStmtList().at(k)) {
                        //In case pointers are not equal compare strings; UGLY!
                        std::string lhsString = PrintStmt::toString(path1->getStmtList().at(k));
                        std::string rhsString = PrintStmt::toString(path2->getStmtList().at(k));
                        isEqual = lhsString == rhsString;
                    }
                }
