...
SCAM::DataPathOptimizer::DataPathOptimizer(const std::vector<SCAM::Assignment *> &dataPathList, SCAM::Module *module) :
    ...
    for (auto assignment: dataPathList) {
        ...
        if (assignment->getRhs()->getDataType()->isCompoundType()) {
            this->newDataPathList.push_back(assignment);
            continue;
        }
        if (assignment->getRhs()->getDataType()->isEnumType()) {
            this->newDataPathList.push_back(assignment);
            continue;
        }
        //Try to optimize stmt
        Assignment *newStmt = applyTactics(assignment);
        this->newDataPathList.push_back(newStmt);
        translator.reset();
    }
}
