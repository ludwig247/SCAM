...
public:
    ...
    blocking_out<CPtoME_IF> CtlToMem_port;
    ...
    blocking_out<CtlToALU_IF> CtlToALU_port;
    blocking_in<ALUtoCtl_IF> ALUtoCtl_port;
    ...
    enum Sections {
        ...
        readRegisterFile,   // read register file data+5
...
void Cpath::run() {
    ...
    while (true) {
        ...
        if (section == Sections::fetch) {
            ...
            cout << endl << "==========================================================================" << endl;
            cout << "@Cpath: Fetching new instruction from location 0x" << hex << memoryAccess.addrIn << endl;
#endif
            ...
            // Addi $0,$0,0 (NOP) is read. Just for debug
            if (encodedInstr == 0x0 || encodedInstr == 0x13) {
#if !defined(NDEBUG) && !defined(TEST)
